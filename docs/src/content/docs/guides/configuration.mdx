...
#### Auto-creating tags with `'create'`
...
In `'create'` mode, spaces and underscores in frontmatter tags get normalized to hyphens for the tag ID, but the original string is kept as the label. For example, `"Web Development"` produces tag ID `web-development` with label "Web Development".
:::

:::note[No `tags.yml` required]
With `'create'` mode, the `tags.yml` file becomes entirely optional. If it doesn't exist, all tags are created from frontmatter. You can still provide a `tags.yml` to customize specific tags while letting the rest be auto-created.
:::
