...
import {
  ...
  isInitialized,
  resetTagsStore
} from 'virtual:starlight-tagging/tags';
...
// Page-based queries
const pageTags = getTagsForPage('guides/authentication'); // Tags for a specific page

// Educational queries
const beginnerTags = getTagsByDifficulty('beginner');     // Filter by difficulty
const tutorialTags = getTagsByContentType('tutorial');    // Filter by content type
const path = getLearningPath('basics', 'advanced');       // Learning path between tags

// Validation & utilities
const { isValid, errors } = validatePrerequisites();      // Validate prerequisite refs
const ready = isInitialized();                            // Check if store is initialized
```
