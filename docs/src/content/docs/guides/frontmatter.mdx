...
An array of tag IDs to associate with the page. By default, tag IDs must match those defined in your `tags.yml` configuration file. With [`onInlineTagsNotFound: 'create'`](/starlight-tags/guides/configuration/#auto-creating-tags-with-create), tags are auto-created if not already defined.
...
When you use a tag that isn't defined in your `tags.yml`, the plugin's behavior depends on your [`onInlineTagsNotFound`](/starlight-tags/guides/configuration/#oninlinetagsnotfound) configuration:

- **`'warn'`** (default): Logs a warning during build
- **`'error'`**: Fails the build
- **`'ignore'`**: Silently ignores undefined tags
- **`'create'`**: Auto-creates the tag from the frontmatter value

:::tip[Spaces in tag names]
With `'create'` mode, you can use natural names like `"Web Development"` in frontmatter. The plugin normalizes them to valid tag IDs (e.g., `web-development`) while keeping the original string as the display label.
:::
