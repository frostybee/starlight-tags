...
3. **Localization:** Custom fields can also support multiple languages using the same object format as `label` and `description`. See the [Internationalization guide](/starlight-tags/guides/i18n/) for the localized format pattern.

4. When accessing tags with custom fields, cast to your custom type:
   ```typescript
...
5. Zod validates your `tags.yml` against the schema at build time. Invalid custom field values will cause build errors.

6. Custom fields are optional, so handle cases where they might be `undefined` in your components.

## Next Steps

- [See the live demo](/starlight-tags/demos/extended-schema/) for a working example with product documentation fields
