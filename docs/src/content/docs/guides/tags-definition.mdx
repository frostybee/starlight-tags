...
To use starlight-tags, you can create a `tags.yml` file that defines all available tags for your documentation site. This file serves as the schema for your tags, specifying their metadata, styling, and optional advanced properties.

:::note
The `tags.yml` file is optional when using [`onInlineTagsNotFound: 'create'`](/starlight-tags/guides/configuration/#auto-creating-tags-with-create). In that mode, tags are created on the fly from frontmatter with minimal metadata. Use `tags.yml` when you need full control over tag appearance and properties.
:::
...
// astro.config.mjs
starlightTagsPlugin({
  onInlineTagsNotFound: 'warn', // 'ignore' | 'warn' | 'error' | 'create'
})
```
...
| `'create'` | Auto-create tags from frontmatter (no `tags.yml` required) |

Using `'warn'` or `'error'` helps catch typos early and keeps your taxonomy consistent. Use `'create'` for a more flexible workflow where tags are defined directly in frontmatter. See the [configuration guide](/starlight-tags/guides/configuration/#auto-creating-tags-with-create) for details.
...
### Localized Labels and Descriptions (i18n)

For multilingual sites, both `label` and `description` can be localized by providing an object with locale keys instead of a simple string:
...
tags:
  ...
  components:
    ...
    description:
      ...
      es: "Componentes de interfaz de usuario"
    color: "#3b82f6"
    icon: "🧩"
```

You can mix both formats in the same file. Tags with simple string values display the same text for all locales, while localized tags show the appropriate translation based on the current page language.

:::tip[Full i18n Details]
See the [Internationalization guide](/starlight-tags/guides/i18n/) for complete details on localization, including fallback behavior and customizing UI translations.
:::
