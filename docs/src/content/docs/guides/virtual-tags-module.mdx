---
title: Using the Virtual Tags Module
...
This function is memoized, so calling it multiple times only initializes once.
...
Get all tags sorted by priority (descending), then count (descending), then label (ascending).
